...
const paginate = async (
	...
	if (start_page > pages.length || start_page < 1) {
		console.error(
			`Start Page must be in range ! (start_page: ${start_page}; pages.len: ${pages.length})`
		)
		interaction.reply(
			'The bot is having a fever, please ask Looz to send koolaid'
		)
		return
	}
	...
	const get_page_payload = (idx: number) => {
		if (pages[idx] instanceof EmbedBuilder) {
			pages[idx].setFooter({
				text: embed_pagination_footer
					.replace('{n}', idx + 1 + '')
					.replace('{max}', pages.length + ''),
			})
			return {
				embeds: [pages[idx]],
				content: null,
     ...
     * @param u user object
     */
	const dispatchPaging = (
		action: 'go_first' | 'go_prev' | 'go_next' | 'go_last',
		r: any,
		u: any,
		event_type: 'collect' | 'remove'
	) => {
