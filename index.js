...
const roll = (message) => {
  ...
  message.channel.send(
    `<@${message.author.id}> rolled a ${Math.floor(Math.random() * max - min)}`
  );
};
...
const tellajoke = (message) => {
  ...
  if (args[1] && args[1] == "dirty") {
    url = "https://www.reddit.com/r/DirtyJokes/.json?sort=hot&t=week&limit=300";
  } else {
    url =
  ...
  fetch(url)
    ...
    .then((json) => {
      try {
        let len = Math.floor(Math.random() * (json.data.children.length - 1));
        message.channel.send(
...
const cursedfood = (message) => {
  fetch("https://www.reddit.com/r/cursedfoods/.json?sort=hot&t=week&limit=800")
    .then((res) => res.json())
    .then((json) => {
      try {
        let len = Math.floor(Math.random() * (json.data.children.length - 1));
        ...
        console.log("Error getting post from r/CursedFood, contact Looz !");
      }
    });
};
  ...
  setTimeout(() => {
    try {
      ...
      for (let i = 0; i < answers.length; i++) {
        if (
          answers[i].children[0].children[0].data.trim().length != word.length
        ) {
          continue;
        }
...
const anime = (message) => {
  let anime_name = message.content.substring(7, message.content.length).trim();
  if (!anime_name) {
...
const tomana = (message) => {
  ...
  if (!isNaN(parseInt(args[1]))) {
    ...
    if (!toram.raw_data[parseInt(args[1])]) {
      message.channel.send(`Sorry, I couldn't find map with id \"${args[1]}\"`);
      return;
    }
    source = parseInt(args[1]);
  } else if (toram.get_idx_from_name(args[1]) == -1) {
    message.channel.send(`Sorry, I couldn't find map with name \"${args[1]}\"`);
    return;
  } else {
  ...
  if (!isNaN(parseInt(args[2]))) {
    ...
    if (!toram.raw_data[parseInt(args[2])]) {
      message.channel.send(`Sorry, I couldn't find map with id \"${args[2]}\"`);
      return;
    }
    dest = parseInt(args[2]);
  } else if (toram.get_idx_from_name(args[2]) == -1) {
    message.channel.send(`Sorry, I couldn't find map with name \"${args[2]}\"`);
    return;
  } else {
...
bot.on("message", (message) => {
  ...
  if (this_msg.startsWith(".")) {
    if (this_msg.startsWith(".ualive")) {
      ...
      message.channel.send("", {
        files: ["https://media1.giphy.com/media/WrP4rFrWxu4IE/source.gif"],
      });
    } else if (this_msg.startsWith(".rekt")) {
      message.channel.send("", {
        files: ["https://media.giphy.com/media/vSR0fhtT5A9by/giphy.gif"],
      });
